...
class TrainingDatasetService:
    ...
    def _setup_hadoop_env(self):
        ...
        os.environ["ARROW_LIBHDFS_DIR"] = f"{settings.HADOOP_HOME}/lib/native"
        
        class_path = ":".join([
            os.environ.get("CLASSPATH", ""),
            os.popen("hadoop classpath").read().strip(),
            f"{settings.HADOOP_HOME}/client/*",
    ...
    def download_file_from_hdfs(self, td_id: str, hdfs_path: str, dataset_format: str) -> bool:
        ...
        try:
            ...
            hdfs_fs = fs.HadoopFileSystem(
                kerb_ticket=settings.TICKET_CACHE_PATH,
                host=settings.HDFS_NAMENODE,
                port=8020
            )
            ...
            # Download file
            local_destination = os.path.join(settings.LOCAL_DIR, f"{td_id}.{dataset_format}")
    ...
    def generate_report(self, td_id: str, dataset_format: str) -> bool:
        ...
        try:
            ...
            if dataset_format == 'tfrecord':
                tfrecord_path = os.path.join(settings.LOCAL_DIR, f"{td_id}.tfrecord")
                df = pdtf.tfrecords_to_pandas(tfrecord_path)
            else:
                file_path = os.path.join(settings.LOCAL_DIR, f"{td_id}.{dataset_format}")
                df = pd.read_csv(
                    file_path, 
                    engine='python', 
                    sep=None, 
            ...
            json_path = os.path.join(settings.LOCAL_DIR, f"{td_id}.json")
    ...
    def upload_json_file(self, td_id: str) -> bool:
        ...
        try:
            ...
            url = f"{settings.URL3}{td_id}/dataprep"
